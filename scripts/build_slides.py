...
SLIDES = [
    ...
    dict(kind="text", label="Part II · The geometry of correlation", title="What we assume about the noise",
         equation=r"\mathbb{E}[\varepsilon \mid X] = 0, \qquad \operatorname{Cov}(\varepsilon \mid X) = \sigma^2 I_n",
         bullets=["Cov means covariance matrix: Cov(Z | X) = E[(Z − E[Z | X])(Z − E[Z | X])ᵀ | X].",
                  "Diagonal entries are variances: Var(εᵢ | X) = σ² for every observation.",
                  "Off-diagonal entries are pairwise covariances: Cov(εᵢ, εⱼ | X) = 0 for i ≠ j.",
    ...
    dict(kind="text", label="Part III · What makes an estimate good", title="Least squares: make the leftovers small",
         equation=r"\hat\beta = \arg\min_{B\in\mathbb R^p} \|Y-XB\|^2 \;\Longrightarrow\; X^{\top}X\hat\beta=X^{\top}Y",
         bullets=["β is the true coefficient vector. It stays fixed and unknown.",
                  "B is a candidate vector varied during minimization; the winning candidate is named β̂.",
                  "The gradient with respect to B is −2XᵀY + 2XᵀXB. Setting it to zero gives the normal equations.",
                  "If XᵀX is invertible, β̂ = (XᵀX)⁻¹XᵀY is the unique solution."]),
    ...
    dict(kind="text", label="Part III · What makes an estimate good", title="The Gauss–Markov theorem",
         equation=r"\operatorname{Var}(a^{\top}\hat\beta\mid X) \le \operatorname{Var}(a^{\top}\tilde\beta\mid X) \quad \text{for every } a\in\mathbb R^p",
         bullets=["The comparison class is β̃ = CY with CX = I: linear in Y and unbiased for every β.",
                  "The vector a selects a direction: Var(aᵀβ̂ | X) = aᵀCov(β̂ | X)a. Setting a = eⱼ compares coefficient j.",
                  "Write C = (XᵀX)⁻¹Xᵀ + D. Because DX = 0, the variance difference is σ²‖Dᵀa‖² ≥ 0.",
